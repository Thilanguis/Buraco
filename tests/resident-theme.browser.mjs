...
try {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 768, height: 1024 }]) {
    ...
    const requested = [];
    page.on('request', r => { if (r.url().includes('/assets/resident/')) requested.push(r.url()); });
    ...
    await page.evaluate(() => { document.body.dataset.deckTheme = 'resident'; document.body.dataset.tableTheme = 'resident'; });
    await page.waitForLoadState('networkidle');
    assert.deepEqual(await dimensions(), before);
    assert.deepEqual((await semanticColors()).slice(0, 2), ['#d6ae67', '#d76b70']);
    assert.equal((await semanticColors())[2], originalSemanticColors[2], 'warnings remain unchanged');
    ...
    assert.deepEqual(palette, ['#d9b775', '#d0c6c6', 'rgba(220, 55, 65, .14)', '.9', 'none', 'none']);
    const result = await page.evaluate(async () => {
      const arts = [...document.querySelectorAll('.resident-card-art')];
      const sources = arts.map(e => getComputedStyle(e).backgroundImage.match(/url\("?(.*?)"?\)/)[1]);
      await Promise.all([...new Set(sources), ...['back-blue', 'back-red', 'table'].map(n => `/assets/resident/${n}.webp`)].map(src => new Promise((resolve, reject) => {
